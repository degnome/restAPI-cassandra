...
var Service = function(configuration) {
  ...
  var readItem = function(args) {
    table.first(args, function(err, result) {
      if(err) {
  ...
  var updateItem = function(args) {
    var scrubedItem = new User(args);
    table.updateOnly(scrubedItem, args.id, function(err, result){
      if(err || !result) {
        return self.emit('send-error', err, 'Failed to Update Item');
      }
      return self.emit('send-data', args);
    });
  };

  // DELETE
  var deleteItem = function(args) {
    table.destroy(args.id, function(err, result) {
      if(err) {
      ...
      if(result === 1) {
        ...
        return self.emit('send-date', false);
      }
    });
  };
